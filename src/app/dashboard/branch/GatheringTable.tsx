...
const GatheringTable = ({user}: {
    user: IUser
  }) => {
    const {data, isLoading, error} = useSWR('/gathering-point', fetcher)
    console.log(data);
    const titles = [
      <p key={0}>ID</p>,
      <p key={1}>Tên</p>,
    ...
    return (
        isLoading ? <Loader/> : <Table titles={titles} data={createTableData(data)} numberPerPage={4} useFooter={true}/>
    )
}
